...
/**
 * 虎扑足球 首页 红色标题部分parser
 * @author haoxiang.jiang@unison.com.cn
 * @date 2014-3-11 下午5:05:34
 */
@Service
public class HupuSoccerFocusTextDivParser implements NewsParser{
	
	private static final Logger logger = LoggerFactory
			.getLogger(HupuSoccerFocusTextDivParser.class);
	
	@Override
	public List<ExtractNews> parse() {
		String baseUrl = "http://soccer.hupu.com/";
		logger.info("开始抽取新闻:baseUrl={}",baseUrl);
		...
		try {
			...
			if(e instanceof SocketTimeoutException){
				logger.error("抽取新闻url={} 失败,网络连接超时！！！",baseUrl,e);
			} else {
				logger.error("抽取新闻url={} 失败！！！",baseUrl,e);
			}
			return null;
		}
	}
	...
	private List<ExtractNews> parseWithElements(Elements elements) {
		...
		for (int i = 0; i < elements.size(); i++) {
			...
			extractNews.setNewsType(NewsType.FootBall.name());

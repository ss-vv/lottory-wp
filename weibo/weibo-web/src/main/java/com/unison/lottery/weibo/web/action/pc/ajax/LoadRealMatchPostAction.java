 ...
 * @version V1.0
 */
public class LoadRealMatchPostAction extends BaseAction{
	...
	public String execute(){
		try {
			setData(messageService.findAllRealMatchPost(getUserLaicaiWeiboId(), matchId ,timeLine));
	...
	public String load(){
		try {
			setData(messageService.findAllRealMatchPost(getUserLaicaiWeiboId(), matchId ,pageRequest));
			data.setUserId(getUserLaicaiWeiboId());
		} catch (Exception e) {
			data.fail("查询赛事相关微博出错！");
			logger.error("{}",data.getDesc(),e);

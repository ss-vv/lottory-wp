...
JAVACMD="java $JAVA_OPTS -classpath conf:$cp core.WeiboStatisticLauncher"

if [ -z "$RUNFILE" ]; then
   RUNFILE="/dev/shm/weibo-statistic"
fi
...
invoke_start(){
    ...
    echo "Starting"
    echo '' > $RUNFILE;
        $JAVACMD 2>/data/logs/weibo-statistic-error.log  & 
    exit "$?" 
}

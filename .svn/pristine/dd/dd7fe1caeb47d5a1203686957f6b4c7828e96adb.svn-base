...
cd /app/news-extract/news-extract-1.0.0-SNAPSHOT
...
JAVACMD="java $JAVA_OPTS -classpath conf:$cp com.unison.lottery.newsextract.main.LauncherNewsExtract"

if [ -z "$RUNFILE" ]; then
   RUNFILE="/dev/shm/news-ext"
fi
...
invoke_start(){
    ...
    echo "Starting"
    echo '' > $RUNFILE;
        $JAVACMD 2>/data/logs/news-extract-error.log  & 
    exit "$?" 
}

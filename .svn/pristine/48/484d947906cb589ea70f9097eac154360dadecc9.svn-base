 ...
 * @author langhsu
 */
public class ZC0401Worker extends AbstractZCDataWorker {
	...
	public void crawl(Parser parser) throws ParserException {
		...
		if (data.size() < 1) {
			log.warn(" -- [ 04_ZC_1 ] data is empty !");
		}
	...
	private ZCOdds parseRow(NodeList cells) {
		if (cells.size() == 13) {
			ZCOdds fb = new ZCOdds(Constants.PLAY_04_ZC_1, "33,31,30,13,11,10,03,01,00");
			try{

...
{
	var data = {};
	data["leagueinfoPO.id"] = id;
	$.post(basePath_+"/footballleagueinfo.do", data, function(ret) {
		if(ret.success == false) {
			alert("查询详情失败");
		} else {
			var dat = ret.data;
			$("#leagueId").text(dat.leagueId);          
			$("#chineseNameAll").text(dat.chineseNameAll);
			$("#chineseName").text(dat.chineseName);
			$("#color").text(dat.colour); //
			//编辑框
			$("#leagueId_edit").text(dat.leagueId);          
			$("#edit_chineseNameAll").text(dat.chineseNameAll);
			$("#edit_chineseName").text(dat.chineseName);
			$("#color_edit").val(dat.colour); 
			$("#color").css("background-color",dat.color);
			$("#color").css("color","#FFFFFF");
			
			switch(dat.countryId)
			{
			case 1:
				$("#countryId").text("欧洲联赛");
...
function(){
    ...
    inputtxt.blur(
    		function()
    		{
    			...
    			var data = {};
    			//alert(select_id);
    			data["leagueinfoPO.id"] = select_id;
    			data["key"] = span.attr("dataname");
    			//alert(span.attr("dataname"));
    			data["newdata"] = newtext;
    			//alert("修改为"+newtext);//    根据id ajax 修改字段数据  footballleagueedit
    			$.post(basePath_+"/footballleagueedit.do", data, function(ret) {
    				if(ret.success == false) {
    					alert("查询详情失败");
    				} else {
    					$('#okinfo').click();
...
//$(document).ready(function () {
//	$("#close_dig").bind('click', function() 
//	{
//		$( "#detail_app_dialog_edit" ).dialog({

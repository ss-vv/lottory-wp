...
/**
 * Copy from JOhm.
 * Dao access the redis with a specific key.
 * 
 * @author Yang Bo
 * @param <T> Model class
 */
public class RedisDao<T> {
    ...
    /** Create a new RedisDao with current key. */
    public RedisDao<T> fork() {
        return new RedisDao<T>(key());
    }
    ...
    public RedisDao(String key) {
        this.key = key;
    }

    public RedisDao(Class<T> clazz) {
        this.key = clazz.getSimpleName();
    }

    public RedisDao(T model) {
    ...
    public RedisDao<T> cat(Object field) {
        prefix();
        sb.append(field);
        sb.append(COLON);
        return this;
    ...
    public RedisDao<T> cat(String field) {
        prefix();
        sb.append(field);
        sb.append(COLON);
        return this;
    ...
    private void checkRedisLiveness() {
        if (jedisPool == null) {
            throw new DaoException(
            		"RedisDao will fail to do most useful tasks without Redis");
        }
    }
}

...
public interface BetSchemeService {
	 ...
	 * @param id
	 */
	void presetAward(int operator, List<Long> id);

	BetScheme getPresetScheme(Long id);

	List<Ticket> listPresetTicket(long schemeId);

	List<BetPartner> listPresetPartner(Long schemeId);

	List<BetScheme> listPresetFollowingScheme(Long showingSchemeId);
	 ...
	 * @param passType
	 */
	void listBetSchemeOrderByOfftimeAsc(Paging paging, Date from, Date to,
			int state, String lotteryId, Long schemeId, String sponsor,

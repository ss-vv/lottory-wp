 ...
 * @author langhsu
 */
public class LC0801Worker extends AbstractLCDataWorker {
	...
	public void crawl(Parser parser) throws ParserException {
		...
		//persist
		if (data.size() < 1) {
			log.warn(" -- [ 08_LC_1 ] data is empty !");
		}
	...
	private LCOdds parseRow(NodeList cells) {
		if (cells.size() == 11) {
			try {
				...
				LCOdds lc = new LCOdds(Constants.PLAY_08_LC_1, "11,01,12,02,13,03,14,04,15,05,16,06");

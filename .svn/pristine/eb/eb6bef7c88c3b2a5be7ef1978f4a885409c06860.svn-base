...
//                	if(!(options.txtAreaObj instanceof jQuery)){
//                		return ;
//                	}
                	if(!options.ueditor){
...
//                    e.stopPropagation();
                    ...
                    for( var i = 0; i < options.emotions.length; i++) {
                        faceHtml += '<li text=' + options.emotions[i].phrase + ' type=' + i + '><img title=' + options.emotions[i].phrase + ' src="http://www.davcai.com/images/emotions/'+ options.emotions[i].url + '"  style="cursor:pointer; position:relative;"   /></li>';
                    }
                    faceHtml += '</ul></div>';
                    ...
                    if(!options.ueditor){
	                    container.find("#face_detail ul >li").bind("click", function(e) {
                    	 ...
                    	 container.find("#face_detail ul >li").bind("click", function(e) {
                             var faceText = $(this).attr("text");
                             options.ueditor.execCommand('inserthtml', faceText);
                             container.find("#face").remove();
                         });
                    }
                    //关闭表情框
                ...
                if(rs) {
                    for( i = 0; i < rs.length; i++) {
                        for( n = 0; n < em.length; n++) {
                            if(em[n].phrase == rs[i]) {
                                var t = "<img src='http://www.davcai.com/images/emotions/"  + em[n].url + "' />";

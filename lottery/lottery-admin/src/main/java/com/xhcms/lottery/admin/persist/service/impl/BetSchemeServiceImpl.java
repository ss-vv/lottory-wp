...
public class BetSchemeServiceImpl implements BetSchemeService {
	...
	@Override
	@Transactional(readOnly = true)
	public List<Ticket> listPresetTicket(long schemeId) {
		List<TicketPresetPO> plist = ticketPresetDao.findByScheme(schemeId, -1);
		if(null != plist && !plist.isEmpty()){
			return POUtils.copyBeans(plist,Ticket.class);
		}
		return null;
	}
...
//		            totalBonus = totalBonus.subtract(share);
			...
			//晒单跟单奖励活动
			if (betSchemePo.getType() == EntityType.BETTING_ALONE
					&& betSchemePo.getShowScheme() == EntityType.SHOW_SCHEME) {
				showSchemeService.awardedShowFollowGrant(operator, betSchemePo);
			}
        }
	...
	@Override
	@Transactional
	public void presetAward(int operator, List<Long> id) {
		Date date = new Date();
		List<BetSchemePO> pos = betSchemeDao.find(id);
		List<BetSchemePresetPO> pops = betSchemePresetDao.findByIds(id);
		if(null == pos || pos.isEmpty() || null == pops || pops.isEmpty()){
			return;
		}
		if(pos.size() != pops.size()){
			logger.error("预派奖方案或源方案数量不匹配！schemeIds:" + id.toString());
			throw new RuntimeException("预派奖方案或源方案数量不匹配！schemeIds:" + id.toString());
		}
		Map<Long, BetSchemePO> smap = new HashMap<Long, BetSchemePO>();
		for(BetSchemePO ps : pos){
			smap.put(ps.getId(), ps);
		}
		Set<Long> sid = new HashSet<Long>();
		for (BetSchemePresetPO presetSchemePo : pops) {
			BetSchemePO sourceSchemePo = smap.get(presetSchemePo.getId());
			if(null == sourceSchemePo){
				logger.error("源方案不存在！派奖失败！schemeId:" + presetSchemePo.getId());
				throw new RuntimeException("源方案不存在！派奖失败！schemeId:" + presetSchemePo.getId());
			}
			// 验证状态
			if(PresetAward.Is_PresetAward.getValue() != sourceSchemePo.getIsPresetAward()){
				logger.error("源方案不是预派奖方案！派奖失败！schemeId:" + presetSchemePo.getId());
				throw new RuntimeException("源方案不是预派奖方案！派奖失败！schemeId:" + presetSchemePo.getId());
			}
			if(EntityStatus.TICKET_NOT_AWARD != presetSchemePo.getStatus() 
					|| EntityStatus.TICKET_NOT_AWARD != sourceSchemePo.getStatus()){
				logger.error("预派奖方案状态错误！方案id={}，原方案状态={},预派奖方案状态={}",new Object[]{sourceSchemePo.getId(),
						sourceSchemePo.getStatus(),presetSchemePo.getStatus()});
				continue;
			}
			sourceSchemePo.setAfterTaxBonus(presetSchemePo.getAfterTaxBonus());
			sourceSchemePo.setStatus(EntityStatus.TICKET_AWARDED);
			presetSchemePo.setStatus(EntityStatus.TICKET_AWARDED);
			BetSchemePO betSchemePo = new BetSchemePO();
			BeanUtils.copyProperties(presetSchemePo, betSchemePo);
			...
			if(betSchemePo.getType() == EntityType.BETTING_PARTNER){
				BigDecimal share = totalBonus.multiply(NumberUtils.percent(betSchemePo.getShareRatio())).setScale(2, RoundingMode.DOWN);
				if(share.compareTo(BigDecimal.ZERO) == 1){
					BetPartnerPresetPO partnerPo = betPartnerPresetDao.get(betSchemePo.getId(), betSchemePo.getSponsorId());
					accountService.giveCommission(operator, betSchemePo.getSponsorId(), 
							share, partnerPo.getId());
...
//		            totalBonus = totalBonus.subtract(share);
			...
			// 给合买人派奖
			HashMap<Long, WinPO> wins = new HashMap<Long, WinPO>();
			for (BetPartnerPresetPO ppo : betPartnerPresetDao.findBySchemeId(betSchemePo.getId())) {
				WinPO win = wins.get(ppo.getId());
			...
			BeanUtils.copyProperties(betSchemePo, presetSchemePo);
		}
		ticketPresetDao.updateStatusByScheme(sid, EntityStatus.TICKET_NOT_AWARD, EntityStatus.TICKET_AWARDED, "派奖");
    ...
    @Override
    @Transactional(readOnly = true)
    public BetScheme getPresetScheme(Long id) {
        BetSchemePO po = getPresetSchemeService.getRightSchemeById(id);
    ...
    public List<BetMatch> listMatch(Long schemeId) {
        ...
        for(BetMatchPO po: pos){
            ...
            if(StringUtils.isNotBlank(po.getPlayId())) {
            	matchIds.add(po.getMatchId());
            	pm.setPlayId(po.getPlayId());
            }
            pm.setId(po.getId());
            ...
            pm.setDefaultScore(po.getDefaultScore());
			pm.setConcedePoints("" + po.getDefaultScore());//设置投注时的让球数
        ...
        if(Constants.JCZQ.equals(spo.getLotteryId())){
            // 竞彩足球
            List<FBMatchPlayPO> fs = null;
            if(matchIds.size() > 0) {
            	fs = fbMatchPlayDao.findByMatches(matchs.values());
            } else {
            	fs = fbMatchPlayDao.find(spo.getPlayId(), matchs.keySet());
            }
            ...
            if(matchIds.size() > 0) {
            	fs = bbMatchPlayDao.findByMatches(matchs.values());
            } else {
            	fs = bbMatchPlayDao.find(spo.getPlayId(), matchs.keySet());
            }
    ...
    @Override
    @Transactional(readOnly = true)
    public List<BetPartner> listPresetPartner(Long schemeId) {
    	return POUtils.copyBeans(betPartnerPresetDao.findBySchemeId(schemeId), BetPartner.class);
    }
    ...
    private PlayMatch toPlayMatch(FBMatchPO po, FBMatchPlayPO mpo, PlayMatch pm){
    	...
    	if(StringUtils.isBlank(pm.getPlayId())) {
    		pm.setPlayId(mpo.getPlayId());
    	}
    	playMatchModifyService.modifyFBPlayMatch(po, mpo, pm);
    ...
    private PlayMatch toPlayMatch(BBMatchPO po, BBMatchPlayPO mpo, PlayMatch pm){
    	...
    	this.playMatchModifyService.modifyBBPlayMatch(po, mpo, pm);
	...
	@Override
	@Transactional(readOnly=true)
	public List<BetScheme> listPresetFollowingScheme(Long showingSchemeId) {
		return POUtils.copyBeans(betSchemePresetDao.findFollowSchemes(showingSchemeId), BetScheme.class);
	}
	...
	@Override
	@Transactional
	public void listBetSchemeOrderByOfftimeAsc(Paging paging, Date from,
			Date to, int status, String lotteryId, Long schemeId,
			String sponsor, String playId, String passType) {
		List<BetSchemePO> list = betSchemeDao.findOrderByOfftimeAsc(paging, from, to, status, lotteryId, schemeId, sponsor, playId, passType);
		List<BetScheme> rets = new ArrayList<BetScheme>(list.size());
		BetScheme b = null;
		for (BetSchemePO po : list) {
			b = new BetScheme();

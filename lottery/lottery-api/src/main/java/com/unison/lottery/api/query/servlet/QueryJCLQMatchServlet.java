 ...
 */
@WebServlet("/xml/queryJCLQMatch")
public class QueryJCLQMatchServlet extends AbstractProcessServlet {
     ...
     */
    public QueryJCLQMatchServlet() {
        super();
        
    }
	@Override
	protected String getMethodName() {
		return MethodNames.QUERY_JCLQ_MATCH;
	}
	@Override
	protected String getResponseVOName() {
		return VONames.QUERY_JCLQ_MATCH_RESPONSE_VO_NAME;
	}
	@Override
	protected HaveReturnStatusResponse process(HttpServletRequest request) {
		
		Map<String,String>  paramMap = (Map<String,String>)request.getAttribute(VONames.QUERY_JCLQ_MATCH_VO_NAME);
		User user=(User) request.getAttribute(VONames.USER_VO_NAME);
		QueryBO queryBO=(QueryBO) ctx.getBean("queryBO");
		return queryBO.queryJCLQMatch(paramMap,user);

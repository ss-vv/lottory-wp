 ...
 */
@WebServlet("/xml/security/schemeDetail")
public class SchemeDetailServlet extends AbstractProcessServlet {
     ...
     */
    public SchemeDetailServlet() {
        super();
        
    }
	@Override
	protected String getMethodName() {
		return MethodNames.SCHEME_DETAIL;
	}
	@Override
	protected String getResponseVOName() {
		return VONames.SCHEME_DETAIL_VO_NAME;
	}
	...
	HaveReturnStatusResponse process(HttpServletRequest request) {
		...
		Map<String,String>  paramMap = (Map<String,String>)request.getAttribute(VONames.SCHEME_DETAIL_VO_NAME);
		SchemeDetailBO schemeDetailBO=(SchemeDetailBO) ctx.getBean("schemeDetailBO");
		return schemeDetailBO.schemeDetail(Long.parseLong(user.getId()), paramMap);
	}

 ...
 * @version 1.0.0
 */
public class BetConfirmAction extends BaseAction {
    ...
    public String execute() {
        ...
        if(showScheme == Constants.SHOW_SCHEME) {
        	if(StringUtils.isNotBlank(showSchemeSlogan) 
        			&& showSchemeSlogan.length() > Constant.WeiboContentLength.POST) {
        		addActionError(getText("show.scheme.OverMaxLength"));
        		return INPUT;
        	}
        	scheme.setShowSchemeSlogan(showSchemeSlogan);
        }
        ...
        if (type == EntityType.BET_TYPE_ALONE) {
        	...
        	//总金额
            scheme.setTotalAmount(bet.getNote() * 2);
            resultCode = betSchemeService.betFollow(getUserId(), scheme, bet.getTickets());
        ...
        String weiboContent = "";
        WeiboUser weiboUser = userAccountService.findWeiboUserByLotteryUid(getUserId()+"");
        String nickName = (null == weiboUser) ? null : weiboUser.getNickName();
        if(StringUtils.isNotBlank(nickName)) {
        	if(type == EntityType.BET_TYPE_FOLLOW) {
        		weiboContent = "我跟了  @" + nickName + " 发起的晒单方案。";
        		betSchemeBaseService.publishShowScheme(scheme, getUserId(), weiboContent);
        	} else if(type == EntityType.BET_TYPE_JION_PARTNER) {
        		weiboContent = "我参与了 @" + nickName + " 发起的合买方案， 认购金额：" + buyAmount + "元";
        		betSchemeBaseService.publishGroupBuyWeibo(schemeId, scheme.getBetRecordId(), 
        				getUserId(), buyAmount, weiboContent, scheme.getSponsorId());
        	}
        } else {
        	logger.error("找不到大V彩用户ID={}，对应的微博用户", getUserId());
        }
        
        if(type == EntityType.BET_TYPE_JION_PARTNER) {
        	BetSchemePO po = betSchemeBaseService.getSchemePOById(schemeId);
        	int purchasedAmount = po.getPurchasedAmount();
        	tagService.updateGroupProgressByGroupScheme(schemeId, po.getTotalAmount(), purchasedAmount);
        }

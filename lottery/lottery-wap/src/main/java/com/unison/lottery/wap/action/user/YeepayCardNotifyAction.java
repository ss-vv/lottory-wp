...
public class YeepayCardNotifyAction extends BaseAction {

	private static final long serialVersionUID = -1308979516404725287L;
	...
	public String execute() {			
		...
		if (hmac.equals(newHmac)) {
			if (r1_Code.equals("1")) {
	        	//根据交易状态处理业务逻辑
	        	//判断该笔订单是否已经做过处理
	        	if(!accountService.isDealWithTransactionResult(new Long(p2_Order))){
	        		accountService.comfirmRecharge(0, Long.parseLong(p9_MP), new BigDecimal(p3_Amt), Long.parseLong(p2_Order), "0");
	        		log.info("wap card yeepay notify success!");
	        	}
				...
				accountService.updateRecharge(Long.parseLong(p2_Order), cardStatus);
				inputStream = new ByteArrayInputStream("fail".getBytes());
				log.info("wap card yeepay notify fail !");
			}
		} else {

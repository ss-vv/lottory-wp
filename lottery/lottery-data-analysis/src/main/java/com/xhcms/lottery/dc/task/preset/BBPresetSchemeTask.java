 ...
 */
public class BBPresetSchemeTask extends Job {

	private static final Logger logger = LoggerFactory.getLogger(BBPresetSchemeTask.class);
	...
	protected void execute() {
		try {
			logger.debug("BB Preset Scheme Task execute ...");
			Map<Long, List<Long>> map = betSchemeBaseService.queryPresetSchemeList(LotteryId.JCLQ.name());
			if(null != map && map.size() > 0) {
				logger.info("find {} size bb preset scheme!schemeIds={}", 
			...
			logger.error("executed {}, error={}", BBPresetSchemeTask.class, e);
		}
	}

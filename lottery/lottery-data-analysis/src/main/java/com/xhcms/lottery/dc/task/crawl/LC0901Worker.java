 ...
 * 
 */
public class LC0901Worker extends AbstractLCDataWorker {
	...
	public void crawl(Parser parser) throws ParserException {
		...
		// persist
		if (data.size() < 1) {
			log.warn(" -- [ 09_LC_1 ] data is empty !");
		}
	...
	private LCOdds parseRow(NodeList cells) {
		if (cells.size() == 8) {
			try {
				LCOdds lc = new LCOdds(Constants.PLAY_09_LC_1, "1,2");
				parseMatchCode(lc, cells.elementAt(0));
				parseOfftime(lc, formater, cells.elementAt(3));
				parseDefaultScore(lc, cells.elementAt(4));

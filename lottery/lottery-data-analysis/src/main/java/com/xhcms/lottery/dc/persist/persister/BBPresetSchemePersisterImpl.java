 ...
 */
public class BBPresetSchemePersisterImpl implements Persister<Map<Long, List<Long>>> {
	...
	public void persist(List<Map<Long, List<Long>>> data) {
		logger.info("收到竞彩篮球预兑奖方案请求:data size={}", data.size());
			...
			for(int i=0; i<data.size(); i++) {
					...
					while(iter.hasNext()) {
						try {
							...
							MatchResults matchResults = presetAwardService.computeBBMatchResults(list);

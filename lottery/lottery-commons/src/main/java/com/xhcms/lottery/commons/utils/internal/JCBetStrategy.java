...
import com.xhcms.lottery.commons.util.DigitalCombination;
import com.xhcms.lottery.lang.LotteryId;
import com.xhcms.lottery.lang.PlayType;

/**
 * <p>Title: 竞彩足球胜平负玩法</p>
 * <p>Description: 支持过关方式：单关、标准过关和组合过关</p>
 * <p>Copyright：Copyright (c) 2011</p>
 * <p>Company：XingHui Spirit (Beijing) Technical Co.,Ltd.</p>
 * 
 * @author wanged,Wang lei
 * @version 1.0.0
 */
public class JCBetStrategy implements BetStrategy {
    ...
    public boolean match(String playId) {
        return Pattern.matches("(\\d{2}_(Z|L)C_\\d)|(\\d{3}_(ZCFH|LCFH)_\\d)", playId);
    }

    @Override
    public Bet resolve(BetScheme s) {
    	if(StringUtils.isNotBlank(s.getLotteryId()) &&
    			LotteryId.JCZQ.name().equals(s.getLotteryId()) || 
    			LotteryId.JCLQ.name().equals(s.getLotteryId())) {
    		//获取竞彩官网赛事ID
    		matchPlatformService.jcOfficialMatchIdList(s.getPlayId(), s.getMatchs());
    	}
    	
        Bet bet = createBet(s.getMatchs());
        bet.setPlayId(s.getPlayId());
        bet.setMultiple(s.getMultiple());
        int multi = s.getMultiple();
        PassCaller passCaller = new PassCaller(bet, s.getMatchs());
        if(PlayType.isOneMatchMutiPlayMixBet(s.getPlayId())) {
        	DigitalCombination digitalCom = DigitalCombination.calcElementIndex(s.getMatchs());
        	List<String> excludeIndex = digitalCom.getExcludeIndex();
        	List<String> includeIndex = digitalCom.getIncludeIndex();
        	s.setExcludeIndex(excludeIndex);
        	passCaller.setExcludeIndex(excludeIndex);
        	passCaller.setIncludeIndex(includeIndex);
        }
        ...
        bet.setMaxBonus(bet.getMaxBonus() * multi * 2);
    ...
    private LinkedList<Ticket> resolveTicket(Bet bet, PassCaller passCaller, String passType) {
        ...
        //m串n的m必须大于等于胆码数量
        if(mn[0] < passCaller.getSeedIndexs().size() && !PlayType.isOneMatchMutiPlayMixBet(bet.getPlayId())){
    ...
    private void checkTicket(LinkedList<Ticket> ts, int multi, String passType, String playId){
        ...
        if (StringUtils.isNotBlank(t.getPlayId())){
        	playId = t.getPlayId();
        } else if(PlayType.isOneMatchMutiPlayMixBet(playId)) {
        	if(playId.equals(PlayType.JCZQ_FH.getPlayIdWithPass(false))) {
        		playId = PlayType.JCZQ_HH.getPlayIdWithPass(false);
        	} else if(playId.equals(PlayType.JCLQ_FH.getPlayIdWithPass(false))) {
        		playId = PlayType.JCLQ_HH.getPlayIdWithPass(false);
        	}
        }
        int m = 0;
        while(multi > 0){
            ...
            tk.setPlayId(playId);
            tk.setExpectBonus(t.getExpectBonus());
            tk.setMatches(t.getMatches());
            tk.setMinMatchPlayingTime(t.getMinMatchPlayingTime());
            ts.add(tk);
        }
    }

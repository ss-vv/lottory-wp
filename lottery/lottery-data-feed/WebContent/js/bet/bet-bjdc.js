Jet().$package(function(J){
    ...
    Bet.prototype = {
        init: function(option){
            this.multi = 1;
            this.pass = (option.minBetMc == 1 ? [0] : [0]); // 已选过关方式
        ...
        toBetMatchs: function(){
            ...
            for(var i = 0, len = bms.length; i < len; i++) {
            	//playId:91_BJDC_SPF
            	//matchs:140708041-00131-false,140708042-011310-false
            	//00131，前面三位表示场次号，不够3位会补0;后面表示投注选项，31表示投注的是胜平
            	var bmsCode = bms[i].code;
            	var limit = 3, str = "00";
            	if(bmsCode && bmsCode.length < limit) {
            		bmsCode = str.slice(0, (limit-bmsCode.length)) + bmsCode;
            	}
                r.push(bms[i].id + '-' + bmsCode + bms[i].opts.join('') + '-' + bms[i].seed);
            }
            return r.join(',');
        },
        ...
        resolve: function(){
            ...
            if(this.pass.length > 0){
                ...
                if(bms.length >= this.minBetMc){
                    ...
                    m = r.note * 2;
                    b = Math.round(r.maxBonus * 100 * 0.65)/100;
                }
            }
            this.money = m;

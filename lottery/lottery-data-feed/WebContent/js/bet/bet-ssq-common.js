...
// 获取双色球结果
function aj_ssq_results(){
	$.ajax({
		url: 'http://trade.davcai.com/df/ssq/aj_results.do?max=12&jsonp=?',
		cache: false,
		crossDomain: true,
		dataType: 'jsonp'
		...
		for(var i=0; i<results.length; i++){
			var result_row = compose_result_row(results[i]);
			$('#ssq_results').append($(result_row));
		}
	});
}

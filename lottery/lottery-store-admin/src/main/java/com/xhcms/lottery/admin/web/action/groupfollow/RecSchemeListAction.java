...
public class RecSchemeListAction extends BaseListAction {
	...
	public String execute() throws Exception {
		...
		if (tab.equals(Constants.JCLQ) || tab.equals(Constants.JCZQ)
			...
			lotteryId = tab;
			return listOnSaleShowingBetScheme(tab);
		}
	 ...
	 */
	private String listOnSaleShowingBetScheme(String lotteryId){
		String resultName = "showing";
		
		if ( StringUtils.isBlank(lotteryId) ){
			addActionError("lotteryId不能为空。");
			return resultName;
		}
		betSchemeService.listOnSaleShowingBetScheme(paging, state, lotteryId, 
				schemeId, sponsor, playId, passType);
		return resultName;
	}
	
	private String listRecommendedScheme(){
		betSchemeService.listRecommendedBetScheme(paging, isOnSale);
		return SUCCESS;
	}

...
public class RegisteServiceImpl implements IVerifyService {
	    ...
	    public String generateCode(long uid, String target) {
	        VerifyPO verify = verifyDao.get(target, UCEntityType.VERIFY_REGISTE);
	        ...
	        if (verify == null) {
	            ...
	            verify.setType(UCEntityType.VERIFY_REGISTE);
	    ...
	    public void verify(long uid, String code) throws  VerifyException{
	        VerifyPO verify = verifyDao.get(uid, UCEntityType.VERIFY_REGISTE);
	        if(null==verify){

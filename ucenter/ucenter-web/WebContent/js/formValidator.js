...
$.formValidator = 
{
	...
	setTipState : function(elem, showclass, showmsg) {
		...
		if (showmsg == null || showmsg == "") {
			...
			if (initConfig.tidyMode) {
				...
				var content = $('<div><b class=""></b><p class="pmt"></p></div>');
				var img = $("b", content);
				var msg = $("p", content);
				if(showclass == "onError"){
					img.removeClass().addClass("ios ios16 ios16_no");
					msg.html(showmsg);
					tip.prev("div").addClass("li_err");
					msg.removeClass("c02").addClass("c01");
					tip.html(content.html());
				}else if(showclass == "onCorrect"){
					img.removeClass().addClass("ios ios16 ios16_yes");
					msg.html(showmsg);
					tip.prev("div").removeClass("li_err");
					msg.removeClass("c01").addClass("c02");
					tip.html(content.html());
				}else if(showclass == "onFocus"){
					img.removeClass().addClass("ios ios16 ios16_note");
					msg.html(showmsg);
					tip.prev("div").removeClass("li_err");
					msg.removeClass("c01").addClass("c02");
					tip.html(content.html());
				}else if(showclass == "onLoad"){
					img.removeClass().addClass("ios ios16 ios16_note");
					msg.html(showmsg);
					tip.prev("div").removeClass("li_err");
					msg.removeClass("c01").addClass("c02");
		...
		if (!isValid){
			...
			if(setting.validateType=="AjaxValidator"){
				...
				if($.formValidator.getLength(id) == 0){
					showmsg = (returnObj.errormsg==""? setting.onEmpty : returnObj.errormsg);
				}else{
	...
	ajaxValid : function(returnObj)
	{
		...
		if(setting.type.toLowerCase() == "get"){
			parm = "clientid="+id+"&"+id+"="+srcjo.val();
		}else{
			if(ls_url.indexOf("?") > -1){
				setting.data = "&clientid="+id+"&"+id+"="+srcjo.val();
			}else{
				setting.data = "clientid="+id+"&"+id+"="+srcjo.val();
			}
		}
